...
if MonteCarlo == 0 || montecarlo_geometry == 1
randn('state',0); %use this seed for thesis results. 

%randn('state',100); %use this seed for journal results
end

%else use any seed

%for journal results, for montecarlo geometry, use this seed..so the
%results the different to my thesis..
%randn('state',100);
...
%Sigma_Gravity_noise = 0;



Sigma_Gravity_noise = 0.00035;  %in m/s^2, which is 0.00035/9.81 g approx 36 micro g 

grav_error_noise(i-1) = abs(Sigma_Gravity_noise);%*randn(1));
...
%     
    ...
    %for L1-L5
     Sigma_Iono_noise = 0.4;    
    Sigma_Tropo_noise = 0.4; 
...
%     Sigma_Tropo_noise = 1;    
    ...
    Sigma_Multipath_noise = 0.25;    
    %Sigma_ReceiverMeasurement_noise = 3;
    
   
    %made it smaller as representative of OEM V
    
    Sigma_ReceiverMeasurement_noise = 0.1;  %6 cm for the OEMV, used 7 cm for measurement precision.    
    
    
     %Sigma_ReceiverMeasurement_noise = 3;  %6 cm for the OEMV, used 7 cm for measurement precision.    
    ...
    TotalCarrierSigma = sqrt(Sigma_Ephem_noise^2 + Sigma_Iono_noise^2 + Sigma_Tropo_noise^2 + Sigma_Multipath_noise_carr^2 + Sigma_ReceiverMeasurement_noise_carr^2);
    
    TotalPRSigma  = sqrt(Sigma_Ephem_noise^2 + Sigma_Iono_noise^2 + Sigma_Tropo_noise^2 + Sigma_Multipath_noise_carr^2 + Sigma_ReceiverMeasurement_noise^2);
   ...
   for isat_count = 1:32
    ...
    GPS_ReceiverMeasurement_noise(isat_count,i-1) = Sigma_ReceiverMeasurement_noise*randn(1);
           
    
    GPS_Multipath_noise_carr(isat_count,i-1) = abs(Sigma_Multipath_noise_carr*randn(1));
    GPS_ReceiverMeasurement_noise_carr(isat_count,i-1) = abs(Sigma_ReceiverMeasurement_noise_carr*randn(1));
...
%     %this is if i want to start the gps without any bias, better gps
%     %accuracy
         ...
         %add up the errors      
          %GPSPr_noise(i) =  sqrt(GPS_Ephem_noise(i)^2 + GPS_Iono_noise(i)^2 + GPS_Tropo_noise(i)^2 + GPS_Multipath_noise(i)^2);% + GPS_ReceiverMeasurement_noise(i);
            ...
            GPSPr_noise(isat_count,i) =  abs((GPS_Ephem_noise(isat_count,i)) + abs(GPS_Iono_noise(isat_count,i)) + abs(GPS_Tropo_noise(isat_count,i)) + abs(GPS_Multipath_noise(isat_count,i))) + GPS_ReceiverMeasurement_noise(isat_count,i);
             ...
             %initialise this here so that carrier noise isn't large at
             %i = 3, because its differentiating
            ...
            % GPSCarrier_noise(isat_count,i-1) = abs(TotalCarrierSigma*randn(1));
             
             GPSPr_Rate_noise(isat_count,i) = 0.2*randn(1);
...
% %        SatOrbError(isat_count,3,i) = 3.5*rand
...
%     
   ...
   Airspeed_SensorTruth(i) = Airspeed_truth(i*100+1);
   Airspeed_Sensor(i) = Airspeed_SensorTruth(i) + Airspeed_noise(i) + 3 ;   %round quantizes it to a metre
   ...
   % Airspeed_Sensor(i) = Airspeer_SensorTruth(i);   
    ...
    Alpha_Sensor(i) = Alpha_SensorTruth(i) + Alpha_noise(i) + 2*pi/180 ;
    
   
   
    %Alpha_Sensor(i) = Alpha_SensorTruth(i)  ;
    ...
    Beta_Sensor(i) = Beta_SensorTruth(i) + Beta_noise(i) + 2*pi/180;

  
  
  
  
   %  Beta_Sensor(i) = Beta_SensorTruth(i);% 
    ...
    %model gravity error as white noise
      [grav_error_noise(i),variance,randU] = GaussMarkov_Process(grav_error_noise(i-1),1/3600,Sigma_Gravity_noise,dT);
...
%Sigma_Param_noise = 0; %1 sigma value for 10% ie 0.1
...
% 
% %for thesis, reduced by 10
% 
% An1Bias = (1/10)*0.5;    %m/s^2
% An2Bias = (1/10)*0.45;   %m/s^2
% An3Bias = (1/10)*-0.2;  %m/s^2
% 
% 
% Gn1Bias = (1/10)*0.57*pi/180;  %rad/s
% Gn2Bias = (1/10)*0.40*pi/180;   %rad/s
% Gn3Bias = (1/10)*0.38*pi/180;   %rad/s
...
Sigma_pBias = 0.0552*pi/180;
Sigma_qBias  = 0.0552*pi/180;
Sigma_rBias  =  0.0552*pi/180;
...
%for thesis, reduced by 10
% Sigma_p = (1/10)*0.53*pi/180;
% Sigma_q = (1/10)*0.45*pi/180;
% Sigma_r = (1/10)*0.44*pi/180;
% 
% Sigma_Ax = (1/10)*0.013;
% Sigma_Ay = (1/10)*0.018;
% Sigma_Az = (1/10)*0.01;
...
% 
% 
% Gn1Bias2 = 0.57*pi/180;  %rad/s
% Gn2Bias2 = 0.40*pi/180;   %rad/s
% Gn3Bias2 = 0.38*pi/180;   %rad/s
...
% Sigma_r = Sigma_r/50;
% 
% Sigma_Ax = Sigma_Ax/50;
% Sigma_Ay = Sigma_Ay/50;
% Sigma_Az = Sigma_Az/50;
...
% else
An1Bias(i-1) = abs(Sigma_AxBias*randn(1)*dtINS) ;
An2Bias(i-1) = abs(Sigma_AyBias*randn(1)*dtINS)  ;
An3Bias(i-1) = abs(Sigma_AzBias*randn(1)*dtINS)  ;

Gn1Bias(i-1) = abs(Sigma_pBias*randn(1)*dtINS)  ; 
Gn2Bias(i-1) = abs(Sigma_qBias*randn(1)*dtINS)  ;
Gn3Bias(i-1) = abs(Sigma_rBias*randn(1)*dtINS)  ;
...
% 
% AN2(1,i-1) = Sigma_Ax2*randn(1)*dtINS ;
% AN2(2,i-1) = Sigma_Ay2*randn(1)*dtINS  ;
% AN2(3,i-1) = Sigma_Az2*randn(1)*dtINS  ;
% 
% GN2(1,i-1) = Sigma_p2*randn(1)*dtINS  ; 
% GN2(2,i-1) = Sigma_q2*randn(1)*dtINS  ;
% GN2(3,i-1) = Sigma_r2*randn(1)*dtINS  ;
% 
...
%     
%         
    [An1Bias(i) ,variance,randU] = GaussMarkov_Process(An1Bias(i-1),1/300,Sigma_AxBias,dtINS );
    [An2Bias(i),variance,randU] = GaussMarkov_Process(An2Bias(i-1),1/300,Sigma_AyBias,dtINS );
    [An3Bias(i),variance,randU] = GaussMarkov_Process(An3Bias(i-1),1/300,Sigma_AzBias,dtINS );
    
    [Gn1Bias(i),variance,randU] = GaussMarkov_Process(Gn1Bias(i-1),1/300,Sigma_pBias,dtINS );
    [Gn2Bias(i),variance,randU] = GaussMarkov_Process(Gn2Bias(i-1),1/300,Sigma_qBias,dtINS );
    [Gn3Bias(i),variance,randU] = GaussMarkov_Process(Gn3Bias(i-1),1/300,Sigma_rBias,dtINS );
...
%     
%     [AN2(1,i) ,variance,randU] = GaussMarkov_Process(AN2(1,i-1),1/120,Sigma_Ax2,dtINS );
%     [AN2(2,i),variance,randU] = GaussMarkov_Process(AN2(2,i-1),1/50,Sigma_Ay2,dtINS );
%     [AN2(3,i),variance,randU] = GaussMarkov_Process(AN2(3,i-1),1/23.23,Sigma_Az2,dtINS );
%     
%     [GN2(1,i),variance,randU] = GaussMarkov_Process(GN2(1,i-1),1/1000,Sigma_p2,dtINS );
%     [GN2(2,i),variance,randU] = GaussMarkov_Process(GN2(2,i-1),1/50,Sigma_q2,dtINS );
%     [GN2(3,i),variance,randU] = GaussMarkov_Process(GN2(3,i-1),1/90,Sigma_r2,dtINS );
    
    
        
%         [AN2(1,i) ,variance,randU] = GaussMarkov_Process(AN2(1,i-1),betaINSNoise,Sigma_Ax2,dtINS );
%     [AN2(2,i),variance,randU] = GaussMarkov_Process(AN2(2,i-1),betaINSNoise,Sigma_Ay2,dtINS );
%     [AN2(3,i),variance,randU] = GaussMarkov_Process(AN2(3,i-1),betaINSNoise,Sigma_Az2,dtINS );
%     
%     [GN2(1,i),variance,randU] = GaussMarkov_Process(GN2(1,i-1),betaINSNoise,Sigma_p2,dtINS );
%     [GN2(2,i),variance,randU] = GaussMarkov_Process(GN2(2,i-1),betaINSNoise,Sigma_q2,dtINS );
%     [GN2(3,i),variance,randU] = GaussMarkov_Process(GN2(3,i-1),betaINSNoise,Sigma_r2,dtINS );
...
%changing noises with dyanmics, make it 1 % of true values

ax_ScaleFactor(i) = 0.01*AccelTruth100Hz(1,i);
ay_ScaleFactor(i) = 0.01*AccelTruth100Hz(2,i);
az_ScaleFactor(i) = 0.01*AccelTruth100Hz(3,i);

p_ScaleFactor(i) = 0.01*GyroTruth100Hz(1,i);
q_ScaleFactor(i) = 0.01*GyroTruth100Hz(2,i);
r_ScaleFactor(i) = 0.01*GyroTruth100Hz(3,i);



% 
% 
% if i == (5000 || 5500 || 7000 || 7500)
%     
%     
%     ax_ScaleFactor(i) = ax_ScaleFactor(i) + randn(1)*500000;
%     
%     p_ScaleFactor(i) = p_ScaleFactor(i) + randn(1)*500000;
%     
% end
    ...
    ax_b_INS_50Hz(i) = AccelTruth100Hz(1,i) + AN(1,i) + An1Bias(i) ;  %+ ax_ScaleFactor(i); 
    ay_b_INS_50Hz(i) = AccelTruth100Hz(2,i) + AN(2,i) + An2Bias(i) ; %+ ay_ScaleFactor(i);
    az_b_INS_50Hz(i) = AccelTruth100Hz(3,i) + AN(3,i) + An3Bias(i) ; %+ az_ScaleFactor(i);

    p_INS_50Hz(i) = GyroTruth100Hz(1,i) + GN(1,i) + Gn1Bias(i); %+ p_ScaleFactor(i);
    q_INS_50Hz(i) = GyroTruth100Hz(2,i) + GN(2,i) + Gn2Bias(i); %+ q_ScaleFactor(i);
    r_INS_50Hz(i) = GyroTruth100Hz(3,i) + GN(3,i) + Gn3Bias(i); %+ r_ScaleFactor(i);
%     
%     
%     
%     
...
%  
%  
%     ax_b_INS_50Hz2(i) = AccelTruth100Hz(1,i) ;%+ AN2(1,i) + An1Bias2 ; 
%     ay_b_INS_50Hz2(i) = AccelTruth100Hz(2,i) ;%+ AN2(2,i) + An2Bias2;
%     az_b_INS_50Hz2(i) = AccelTruth100Hz(3,i) ;%+ AN2(3,i) + An3Bias2;
% 
%     p_INS_50Hz2(i) = GyroTruth100Hz(1,i) ;%+ GN2(1,i) + Gn1Bias2;
%     q_INS_50Hz2(i) = GyroTruth100Hz(2,i) ;%+ GN2(2,i) + Gn2Bias2 ;
%     r_INS_50Hz2(i) = GyroTruth100Hz(3,i) ;%+ GN2(3,i) +Gn3Bias2;
%     
%     
%     
    
%     
%     %for seeing what the fusion gives in an equivalent INS
%      ax_b_INS_50Hz(i) = AccelTruth100Hz(1,i) + AN(1,i)/2 + An1Bias ; 
%     ay_b_INS_50Hz(i) = AccelTruth100Hz(2,i) + AN(2,i)/2 + An2Bias;
%     az_b_INS_50Hz(i) = AccelTruth100Hz(3,i) + AN(3,i)/2 + An3Bias;
% 
%     p_INS_50Hz(i) = GyroTruth100Hz(1,i) + GN(1,i)/2 + Gn1Bias;
%     q_INS_50Hz(i) = GyroTruth100Hz(2,i) + GN(2,i)/2 + Gn2Bias;
%     r_INS_50Hz(i) = GyroTruth100Hz(3,i) + GN(3,i)/2 + Gn3Bias;
%     
%     
...
%     
     ...
     AN1Hz(1,i) =  An1Bias(i*100+1) ;
      AN1Hz(2,i) =  An2Bias(i*100+1) ;
        AN1Hz(3,i) =  An3Bias(i*100+1) ;        
          GN1Hz(1,i) =  Gn1Bias(i*100+1) ;
            GN1Hz(2,i) =  Gn2Bias(i*100+1) ;
              GN1Hz(3,i) =  Gn3Bias(i*100+1) ;
              
              
              
              
%                
%    ax_b_INS1Hz2(i) = ax_b_INS_50Hz2(i*100+1);
%     ay_b_INS1Hz2(i) = ay_b_INS_50Hz2(i*100+1);
%     az_b_INS1Hz2(i) = az_b_INS_50Hz2(i*100+1);    
% 
%     p_INS1Hz2(i) = p_INS_50Hz2(i*100+1);
%     q_INS1Hz2(i) = q_INS_50Hz2(i*100+1);
%     r_INS1Hz2(i) = r_INS_50Hz2(i*100+1);       
%        
%     
%     AN1Hz2(1,i) =  AN2(1,i*100+1) ;
%       AN1Hz2(2,i) =  AN2(2,i*100+1) ;
%         AN1Hz2(3,i) =  AN2(3,i*100+1) ;        
%           GN1Hz2(1,i) =  GN2(1,i*100+1) ;
%             GN1Hz2(2,i) =  GN2(2,i*100+1) ;
%               GN1Hz2(3,i) =  GN2(3,i*100+1) ;
%               
%               
%               
