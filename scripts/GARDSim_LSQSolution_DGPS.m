...
% this function calculates teh differentially corrected LSQ solution using
...
for Epoch_lo = 1:NumberGPSEpochs
    ...
    for Obs = 1:NumberGPSMeasurements
        ...
        if(SVDontUse(SV) == 0) 
            ...
            if DGPS_flag == 1
                % check if we have a differential correction to match
                baseEpoch = find (round(gps.RangeData(Epoch_lo).GPSSec) == round(BaseStation.GPSTime_Sec));

                if isempty(baseEpoch)
                   % nothing available at this time
                    disp(sprintf('No PRC avaialble at time %f at Epoch %d',gps.RangeData(Epoch_lo).GPSSec,Epoch_lo));
                   continue; 
                end

                if BaseStation.PRC(SV,baseEpoch) == 0
                    % no prc available
                    disp(sprintf('No PRC avaialble for SV%d at Epoch %d',SV,Epoch_lo));
                    continue;
                end
            end
            ...
            SV_Vec(SVIndex) = SV;
            if DGPS_flag == 1
                PR_Vec(SVIndex) = gps.RangeData(Epoch_lo).Obs(Obs).dPseudorange + BaseStation.PRC(SV,baseEpoch);
            else
    ...
    if exist('PR_Vec','var')
        ...
        disp(sprintf('Epoch %d: No Observations',Epoch_lo));
        continue;
    end

    NumberGPSMeasurementsAvailable(Epoch_lo) = gps.RangeData(Epoch_lo).lNumberObservations;
    SVs_Used(Epoch_lo,1:NumberGPSMeasurements) = SV_Vec(1:NumberGPSMeasurements);
    
    if NumberGPSMeasurements < 4
        disp(sprintf('Epoch %d: Insufficient measurements (%d) for Solution', Epoch_lo,NumberGPSMeasurements));
        continue;
    end
   ...
   NumberGPSMeasurementsUsed(Epoch_lo) = NumberGPSMeasurements;
    ...
    [LSQ_Solution(Epoch_lo,:), LSQ_Variance(Epoch_lo,:), LSQ_NumIterations(Epoch_lo), ...
        ...
        LSQ_limit(Epoch_lo), LSQ_DOP(Epoch_lo,:)] = GARD_LSQ(UserPos,NumberGPSMeasurements, PR_Vec_raw(1:NumberGPSMeasurements),SVPos(1:NumberGPSMeasurements,:),DGPS_flag);
   ...
   [LSQ_Solution_LLH(Epoch_lo,1) LSQ_Solution_LLH(Epoch_lo,2) LSQ_Solution_LLH(Epoch_lo,3)] = ECEF2LLH(LSQ_Solution(Epoch_lo,1:3));
   Tecef2ned = T_ECEF2NED(LSQ_Solution_LLH(Epoch_lo,1),LSQ_Solution_LLH(Epoch_lo,2));
   Tecef2ned2 = Tecef2ned;
   Tecef2ned2(4,1:3) = [0 0 0];
   Tecef2ned2(1:4,4) = [0 0 0 1];
   
   LSQVel_Solution_NED(Epoch_lo,:) = Tecef2ned * LSQVel_SolutionVec(Epoch_lo,1:3)';
   
   
   % find span data for comparisson
   spanEpoch = find (round(gps.RangeData(Epoch_lo).GPSSec) < (span(:,4)),1);
   PosLLH_error_LSQ(Epoch_lo,:) = (LSQ_Solution_LLH(Epoch_lo,:) - span(spanEpoch,5:7) .* [pi/180 pi/180 1]) .* [RM RP*cos(-0.47) 1]; 
