...
TMatrixBody2Wind = T_Body2Wind(alpha0, beta0);

%velocity in body axes
V_b = TMatrixBody2Wind'*[V_T0, 0, 0]';
...
x0(startepoch,:) = [V_T0, beta0 ,alpha0, phi0, theta0, psi0, p0,q0,r0,pn0, pe0, h0, q00, q10, q20, q30, Lat0, Lon0, Hgt0,u,v,w];    
    

for i = startepoch:4900         
...
%calculate state velocity dx
  ...
  %calculate MACH number for next time around
      ...
      [ECEFModel] = LLH2ECEF(LatModel(i),LonModel(i),HgtModel(i));
        ...
        Wind_NED = [0,0,0];
        %GravityTruth(i) = 9.81;
    [dx, AeroCoeffs,Forces, Moments] = AerodynamicModelStevensNew(M0,GravityTruth(i),AtmosTruth(:,i),x0(i,:),delta_x_control(i,:), AeroCoeffsTruth, EngCoeffsTruth,PropCoeffsTruth,ForcesTruth, MomentsTruth, MassTruth,InertiaTruth,CGposTruth,MachTruth, Parameter_Noise, Wind_NED,AeroCoeffCorrections);

     
     
     % x0(i+1,:) = x0(i,:) + ts*dx(1:22);
           
          
      
      
           %integrate runge kutta 
    [k1dx,AeroCoeffs,Forces, Moments] =AerodynamicModelStevensNew(M0,GravityTruth(i),AtmosTruth(:,i),x0(i,:),delta_x_control(i,:), AeroCoeffsTruth, EngCoeffsTruth,PropCoeffsTruth,ForcesTruth, MomentsTruth, MassTruth,InertiaTruth,CGposTruth,MachTruth, Parameter_Noise, Wind_NED,AeroCoeffCorrections);
    [k2dx,AeroCoeffs,Forces, Moments] = AerodynamicModelStevensNew(M0,GravityTruth(i),AtmosTruth(:,i),x0(i,:)+(ts/2)*(k1dx),delta_x_control(i,:), AeroCoeffsTruth, EngCoeffsTruth,PropCoeffsTruth,ForcesTruth, MomentsTruth, MassTruth,InertiaTruth,CGposTruth,MachTruth, Parameter_Noise, Wind_NED,AeroCoeffCorrections);
    [k3dx,AeroCoeffs,Forces, Moments] =AerodynamicModelStevensNew(M0,GravityTruth(i),AtmosTruth(:,i),x0(i,:)+(ts/2)*(k2dx),delta_x_control(i,:), AeroCoeffsTruth, EngCoeffsTruth,PropCoeffsTruth,ForcesTruth, MomentsTruth, MassTruth,InertiaTruth,CGposTruth,MachTruth, Parameter_Noise, Wind_NED,AeroCoeffCorrections);
    [k4dx,AeroCoeffs,Forces, Moments] = AerodynamicModelStevensNew(M0,GravityTruth(i),AtmosTruth(:,i),x0(i,:)+ts*(k3dx),delta_x_control(i,:), AeroCoeffsTruth, EngCoeffsTruth,PropCoeffsTruth,ForcesTruth, MomentsTruth, MassTruth,InertiaTruth,CGposTruth,MachTruth, Parameter_Noise, Wind_NED,AeroCoeffCorrections);
       
    
    
    x0(i+1,:)  =  x0(i,:) + ts*(k1dx/6 + k2dx/3 +k3dx/3 + k4dx/6);  
      
      
      
      
     %update Airpseed with measurements from air data sensor to see if it
     %bounds error
     
%      x0(i+1,1) = Airspeed_truth(i); 
%      x0(i+1,2) =Beta_truth(i);
%      x0(i+1,3) = Alpha_truth(i);
