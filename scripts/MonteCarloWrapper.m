...
Nmc = 10;   %83 runs is the maximum without getting an out of memory error
...
%CLEAR ALL THESE VARIABLES AND UNUSED VARIABLES<

%BUT THE PROBLEM IS WITH PMINUS 100Hz
...
corrcoeffGyrobias_Cmc(iM,:,:) = corrcoeffGyrobias;
  ...
  RollErrorINS_NFmc(iM,:) = RollErrorINS_NF;
   ...
   PitchErrorINS_NFmc(iM,:) = PitchErrorMODEL_NF;
   YawErrorINS_NFmc(iM,:) = YawErrorMODEL_NF; 
...
%feature memstats  %to see the memory status
...
plot(N_PosErrorsINS_Cmc(:,startprocessepoch:endprocessepoch)','y--')
plot(2*sqrt(varN_PosErrorsINS_Cmc(startprocessepoch:endprocessepoch)),'r--')
plot(-2*sqrt(varN_PosErrorsINS_Cmc(startprocessepoch:endprocessepoch)),'r--')
plot(meanN_PosErrorsINS_Cmc(startprocessepoch:endprocessepoch))
plot(2*sqrt(PnC(startprocessepoch:endprocessepoch)),'b--')
