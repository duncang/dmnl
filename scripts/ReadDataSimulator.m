...
%THIS IS TROYS READ SIMULATOR DATA IN, NOT THE SAME AS READSIMULATOR.M
%read in truth data
...
%for i = startepochHighRate:endepochHighRate + (TruthOffset-1)*100 + 1
for i = startepochHighRate:endepochHighRate  %+ (TruthOffset-1)*100;  
    
    k = i;
    
    
      %TruthIndex100Hz = i+TruthOffset*100-1;
    ...
    %XposCalc.
    Xpos_truth(i) = pos_truth_ecef(2,k);
    Ypos_truth(i) = pos_truth_ecef(3,k);
    Zpos_truth(i) = pos_truth_ecef(4,k);
    
    
    Lat_truth(i) = pos_truth_llh(2,k);
    Lon_truth(i) = pos_truth_llh(3,k);
    Hgt_truth(i) = pos_truth_llh(4,k);
        
        
      %velocity truth

    V_n_truth(i) = vel_truth(2,k);
      V_e_truth(i) = vel_truth(3,k);
        V_d_truth(i) = vel_truth(4,k);
...
% 
    ax_b_INS_50Hz(i) = sensors_clean(2,i);
    ay_b_INS_50Hz(i) = sensors_clean(3,i);
    az_b_INS_50Hz(i) = sensors_clean(4,i);
    p_INS_50Hz(i) = sensors_clean(5,i);
    q_INS_50Hz(i) = sensors_clean(6,i);
    r_INS_50Hz(i) = sensors_clean(7,i);
...
%     end
    ...
    Roll_truth(i) = att_truth(2,k);   % bank angle(rad)  PHI
    Pitch_truth(i) = att_truth(3,k);    %pitch angle (rad) THETA
    Yaw_truth(i) = att_truth(4,k);    %heading (rad) PSI
...
% 
% 
% p_INS_50Hz(i*100+1) = 0;
% q_INS_50Hz(i*100+1) = 0;
% r_INS_50Hz(i*100+1) = 0;
% 
% ax_b_INS_50Hz(i*100+1) = 0;
% ay_b_INS_50Hz(i*100+1) = 0;
% az_b_INS_50Hz(i*100+1) = 0;
...
for i = startepoch-1:endepoch
    ...
    %ma = i*100 + (TruthOffset-1)*100 + 1;

    
        ma = i*100 +1 ;%+ (TruthOffset-1)*100 ;%+ 1;
...
%     Hgt_truth1Hz(i) = pos_truth_llh(4,i*100+1);
    ...
    Lat_truth1Hz(i) = Lat_truth(ma);
    Lon_truth1Hz(i) = Lon_truth(ma);
    Hgt_truth1Hz(i) = Hgt_truth(ma);

    Xpos_truth1Hz(i) = Xpos_truth(ma);
    Ypos_truth1Hz(i) = Ypos_truth(ma);
    Zpos_truth1Hz(i) = Zpos_truth(ma);

    Xvel_truth1Hz(i) = Xvel_truth(ma);
    Yvel_truth1Hz(i) = Yvel_truth(ma);
    Zvel_truth1Hz(i) = Zvel_truth(ma);
    
    
      V_n_truth1Hz(i) = V_n_truth(ma);
      V_e_truth1Hz(i) = V_e_truth(ma);
      V_d_truth1Hz(i) = V_d_truth(ma);
    ...
    Roll_truth1Hz(i) = Roll_truth(ma);
    Pitch_truth1Hz(i) = Pitch_truth(ma);
    Yaw_truth1Hz(i) = Yaw_truth(ma);
    
    
    Quaternions_truth1Hz(1,i) = Quaternions_truth(1,ma);
    Quaternions_truth1Hz(2,i) = Quaternions_truth(2,ma);
    Quaternions_truth1Hz(3,i) = Quaternions_truth(3,ma);
    Quaternions_truth1Hz(4,i) = Quaternions_truth(4,ma);
...
%                 
  ...
  %sensor data at 1 Hz
%    
   ax_b_INS1Hz(i) = ax_b_INS_50Hz(ma);
    ay_b_INS1Hz(i) = ay_b_INS_50Hz(ma);
    az_b_INS1Hz(i) = az_b_INS_50Hz(ma);
    

    p_INS1Hz(i) = p_INS_50Hz(ma);
    q_INS1Hz(i) = q_INS_50Hz(ma);
    r_INS1Hz(i) = r_INS_50Hz(ma);       
%        
