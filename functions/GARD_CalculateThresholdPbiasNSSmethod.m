function [a_out_H,a_out_V, lambda_out_H, lambda_out_V] = GARD_CalculateThresholdPbiasNSSmethod(P_fa,P_md_H,P_md_V,DOFrange)
...
a_H = chi2inv(1-P_fa./DOFrange,2);   %added ./DOFrange as this is how the threshold is calculated in normalised SS method 29.1.08
TD_H = sqrt(a_H);
...
for j = 1:numds
    ...
    P_est_H = ncx2cdf(a_H(j),2,lambda_guess_H);
    while (P_est_H > P_md_H)
       lambda_guess_H = lambda_guess_H + 0.01;   %use 0.01 here is a bit more accurate to the second decimal place
       %P_est_H = ncx2cdf(a_H(j),DOFrange(j),lambda_guess_H);
...
for j = 1:numds
    ...
    while (P_est_V > P_md_V)
       lambda_guess_V = lambda_guess_V + 0.01;
       %P_est_V = ncx2cdf(a_V(j),DOFrange(j),lambda_guess_V);
