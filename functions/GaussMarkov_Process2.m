function [Xout,variance] = GaussMarkov_Process2(Xin, Beta,WN_Std_dev,dT);
%Use this with UKF , as the sigma value is passed in the UKF , so randn is
%not used.

%by Troy Bruggemann 
%Modified by Duncan Greer for use with his UKF code
...
%get random sample from uniform distribution
...
Wk = sqrt(variance);%*randU;

Xout = exp(-Beta*dT)*Xin + Wk*dT;
