function [SV_X_Data, SV_Y_Data, SV_Z_Data, SV_T_Data, ValidData] = GPSOrbitPropagatorOptimal(SVWeek, SVTime, SV, NavigationData)
%function [SV_X_Data SV_Y_Data SV_Z_Data SV_T_Data ValidData] = GPSOrbitPropagator(SVWeek, SVTime, SV, NavigationData)
...
% [SV_X_Data SV_Y_Data SV_Z_Data SV_T_Data] = GPSOrbitPropogator(SVRXTime, SV,NavigationData)
%
...
%
%  Written by Duncan Greer (c) QUT GRAS Project 2005
...
% navigation data returned by freadnav:
    ...
    if (DataValid == 1)
        ...
        %delta_tr = 0;  
        delta_tr = F * e * sqrt(A) * sin(Ek); %will be 0 because e is 0.
...
%         %-------------------------------------
        ...
        %tk = 76292736;   %total seconds between reference epoch and epoch u want to calculate for
