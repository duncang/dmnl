function [SolutionVec, VarSolutionVec, NumIterations, ResidualVector, M, LSQ_Fail, limit, DOP] = GARD_LSQ_ClockBias(UserPos,N,PRMeasured,SVPos);


%This function estimates the receiver clock bias only, given the true
...
%
% $Id: GARD_LSQ_ClockBias.m 1850 2008-07-14 04:52:47Z greerd $
%
...
%         
    ...
    for k = 1:N
        M(k,1) =  -(SVPos(k,1) - UserPos(1))/r_VecCalc(k);
        M(k,2) =  -(SVPos(k,2) - UserPos(2))/r_VecCalc(k);
        M(k,3) =  -(SVPos(k,3) - UserPos(3))/r_VecCalc(k);
        M(k,4) = 1;
    end
