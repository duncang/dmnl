...
function [SolutionVec, VarSolutionVec, NumIterations, ResidualVector, M, LSQ_Fail, limit] = GARD_LSQVelUDSTwoMeas(UserPos,UserVel,N,PRMeasured,SVPos, SVVel, Zterm);

global GPS_PI OMEGAedot mu Earthradius Speedoflight c F L1_f L2_f gamma L1_Wavelength;
%Version 1.00
%Troy Bruggemann 30 June 2005

%This function does a least squares code solution for receiver velocities with an underdetermined solution, only 2 measurements single epoch only.
...
% OMEGAedot = 7.2921151467e-5; % Earth rotation rate in radians per second
 ...
 %initialise variables
    ...
    XbVel = UserVel(1);
    YbVel = UserVel(2);
    ZbVel = Zterm;  %this is estimated z so only x and y need to be calculated
    %dtbVel = UserVel(4);
    ...
    N = 2 %only use 2 measurements to calculate velocity.
...
for k = 1:N
            ...
            %x and y components only, only 2 unknowns..xdot and ydot
...
%residual vector
ResVec = PRMeasured(1:2)' - ResCalcVel';
...
limit = sqrt(Delta_X(1)^2 +  Delta_X(2)^2);
