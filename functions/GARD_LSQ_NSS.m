function [SolutionVec, VarSolutionVec, NumIterations, ResidualVector, M, LSQ_Fail, limit, DOP,AA_out] = GARD_LSQ_NSS(UserPos,N,PRMeasured,SVPos,Elev);
...
% THIS IS FOR THE NORM SS METHOD, SAME AS REGULAR LEAST SQUARES I JUST
% OUTPUT THE FULL AA MATRIX SO I GET THE CORRELATIONS
%Version 1.00
...
%Elev Elev is vector of elevations in degrees, so can weight the satellitse
%appropriately


sizeW = length(ResCalc);
    W = eye(sizeW);
...
%for weighted i think u do this
...
var_x = AA(1,1);  %this is variance of north 
var_y = AA(2,2); %east variance
var_z = AA(3,3); %up variance
var_dt = AA(4,4);
