function [SolutionVec, VarSolutionVec, NumIterations, ResidualVector, M, LSQ_Fail, limit, DOP] = GARD_LSQ_GMS(GRSPos,N,PRMeasured,SVPosStart);


%This function estimates the ephemeris error based on position of at least
%4 ground stations. i.e. reverse GPS.
...
%
% $Id: GARD_LSQ_GMS.m 1850 2008-07-14 04:52:47Z greerd $
%
...
for j = 1:200   %Least square iterations counter
    ...
    for k = 1:N
        ...
        for m = 1:3
            ele(m) =  GRSPos(k,m) - TempBase_B_Pos(m);
        end
    ...
    for k = 1:N
        ...
        delta_pr_omegaedot(k) = -(OMEGAedot / Speedoflight) * (GRSPos(k,1) * TempBase_B_Pos(2) - GRSPos(k,2) * TempBase_B_Pos(1));
    ...
    for k = 1:N
        ...
        %%(ResCalc(k))
         ...
         ResCalc(k) =  r_VecCalc(k) - TempBase_B_Pos(4) + GRSPos(k,4) ;%- delta_pr_omegaedot(k);
%         
...
%         
    ...
    for k = 1:N
        M(k,1) =  -(GRSPos(k,1) - TempBase_B_Pos(1))/r_VecCalc(k);
        M(k,2) =  -(GRSPos(k,2) - TempBase_B_Pos(2))/r_VecCalc(k);
        M(k,3) =  -(GRSPos(k,3) - TempBase_B_Pos(3))/r_VecCalc(k);
        M(k,4) = 1;
    end
